...
<template>
  <section class="container-fluid py-4">
    <PageHeader title="Puestos" subtitle="Aquí defines el salario base que heredarán los empleados.">
      <template #action>
        <button class="btn btn-primary" @click="nuevo" data-testid="add-puesto-btn">
    ...
    <DataTable v-else :headers="headers" :empty="!puestos.length" empty-text="Sin puestos">
      <tr v-for="p in puestos" :key="p.id_puesto">
        ...
        <td class="fw-semibold">{{ money(p.salario_base) }}</td>
        <td>
        ...
        <td class="text-end">
          <button class="btn btn-outline-secondary btn-sm me-1" @click="editar(p)">
          ...
          >
            <i :class="['bi', p.activo ? 'bi-ban' : 'bi-check-lg']"></i>
          </button>
        </td>
      </tr>
    </DataTable>
    ...
    <ModalBase v-if="modal" :title="modalTitle" @close="modal = false" @save="guardar">
      <div class="row g-3">
        <div class="col-12">
          ...
          <input class="form-control" v-model="form.nombre" data-testid="puesto-nombre" />
        </div>
        <div class="col-12">
        ...
        <div class="col-12 col-md-4">
          ...
          <select class="form-select" v-model="form.nivel">
        ...
        <div class="col-12 col-md-4">
          ...
          <input type="number" step="0.01" class="form-control" v-model="form.salario_base" data-testid="puesto-salario" />
        </div>
        <div class="col-12 col-md-4">
